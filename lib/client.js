window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Page({ connection }) {
      ...
      const saveProviderApi = async (route, api) => { try { const normalized = normalizeApi(api); if (normalized === null) throw new Error('协议必须是 openai-completions、openai-responses、anthropic-messages，或对应的 /chat/completions、/responses、/messages 路径。'); if (!normalized) { notify('未配置模型级协议时，将保持该提供商原有协议和 API 地址。'); return; } const response = await connection.api.settings.mutate({ ns: 'llm-pi-ai', ops: [{ op: 'set', path: ['providers', route, 'api'], value: normalized }], expectedRevision: llm.revision }); if (!response.result.ok) throw new Error(response.result.error.message); setLlm(response.result.value); notify('提供商默认协议已保存。'); } catch (error) { notify(error.message, 'error'); } };
