window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Page({ connection }) {
      ...
      const subagentView = React.createElement('div', { style: { maxWidth: '760px' } }, React.createElement('p', { style: { color: '#57606a' } }, '配置子 agent 默认模型、角色匹配和未匹配回退策略。规则保存后由兼容的派发适配器使用。'), React.createElement('label', null, '未匹配回退', React.createElement('select', { value: subagentConfig.fallback || 'default', onChange: (e) => setSubagentConfig({ ...subagentConfig, fallback: e.target.value }), style: field }, React.createElement('option', { value: 'default' }, '使用子 agent 默认模型'), React.createElement('option', { value: 'inherit' }, '继承当前主会话模型'), React.createElement('option', { value: 'deny' }, '禁止派发并提示'))), React.createElement('div', { style: { height: '10px' } }), React.createElement('label', null, '子 agent 配置 JSON', React.createElement('textarea', { value: subagentText || JSON.stringify(subagentConfig, null, 2), onChange: (e) => setSubagentText(e.target.value), rows: 18, style: { ...field, fontFamily: 'monospace', resize: 'vertical' } })), React.createElement('p', { style: { color: '#57606a', fontSize: '12px' } }, '角色字段：name、aliases、keywords、priority、selection。selection 支持 provider、model、reasoningEffort、contextWindow、maxTokens、image。'), React.createElement('button', { style: primary, onClick: async () => { try { const next = JSON.parse(subagentText || JSON.stringify(subagentConfig)); if (!Array.isArray(next.roles)) throw new Error('roles 必须是数组。'); const response = await connection.api.settings.mutate({ ns: 'llm-pi-ai', ops: [{ op: 'set', path: ['changmodelSubagents'], value: next }], expectedRevision: llm.revision }); if (!response.result.ok) throw new Error(response.result.error.message); setLlm(response.result.value); setSubagentConfig(next); setSubagentText(''); setNotice('子 agent 配置已保存。'); } catch (error) { setNotice(error.message); } } }, '保存子 agent 配置'));
