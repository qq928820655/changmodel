  ...
  factory: (require) => {
    ...
    function Page({ connection }) {
      ...
      const load = React.useCallback(async () => {
        ...
        fetch('/changmodel/subagent-dispatch').then(readJson).then((result) => { if (result.ok) setDispatchPatch(result.value); else setNotice(result.error || '子 agent 补丁状态检查失败。'); }).catch((error) => setNotice(error.message));
      ...
      const historyView = React.createElement('div', { style: { maxWidth: '600px', border: '1px solid #d8dee4', borderRadius: '8px', padding: '16px' } }, React.createElement('h3', { style: { marginTop: 0 } }, '输入历史回填'), React.createElement('p', { style: { color: '#57606a' } }, '空输入框按上下键浏览当前会话的用户历史文本。启用、重新应用或恢复默认后均需重启 DSH。'), React.createElement('div', { style: { display: 'flex', gap: '8px', flexWrap: 'wrap' } }, React.createElement('button', { style: primary, onClick: async () => { try { const response = await fetch('/changmodel/input-history', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ action: 'apply' }) }); const result = await readJson(response); setNotice(result.ok ? '输入历史回填已启用，请重启 DSH。' : result.error); } catch (error) { setNotice(error.message); } } }, '启用 / 重新应用'), React.createElement('button', { style: secondary, onClick: async () => { try { const response = await fetch('/changmodel/input-history'); const result = await readJson(response); setNotice(result.ok ? (result.value.patched ? '当前状态：已启用。' : '当前状态：未启用。') : result.error); } catch (error) { setNotice(error.message); } } }, '检查状态'), React.createElement('button', { style: { ...secondary, color: '#cf222e', borderColor: '#ffb4b4' }, onClick: async () => { try { const response = await fetch('/changmodel/input-history', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ action: 'restore' }) }); const result = await readJson(response); setNotice(result.ok ? '已恢复默认输入组件，请重启 DSH。' : result.error); } catch (error) { setNotice(error.message); } } }, '恢复默认')));
