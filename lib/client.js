window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Page({ connection }) {
      ...
      return React.createElement('div', { style: { maxWidth: '880px', padding: '6px', fontFamily: 'system-ui, sans-serif' } }, React.createElement('h2', { style: { margin: '0 0 12px' } }, 'changmodel'), React.createElement('div', { style: { display: 'flex', gap: '8px', marginBottom: '16px' } }, React.createElement('button', { style: { ...primary, background: tab === 'models' ? '#1677ff' : '#6b7280' }, onClick: () => setTab('models') }, '模型能力与档位'), React.createElement('button', { style: { ...primary, background: tab === 'summary' ? '#1677ff' : '#6b7280' }, onClick: () => setTab('summary') }, '固定摘要模型'), React.createElement('button', { style: { ...primary, background: tab === 'history' ? '#1677ff' : '#6b7280' }, onClick: () => setTab('history') }, '输入历史回填')), addModal, tab === 'models' ? providersView : tab === 'summary' ? summaryView : historyView, notice ? React.createElement('p', { style: { color: '#57606a' } }, notice) : null);
    }
