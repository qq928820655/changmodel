window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Page({ connection }) {
      ...
      const saveProviderApi = async (route, api) => { try { const normalized = normalizeApi(api); if (normalized === null) throw new Error('协议必须是 openai-completions、openai-responses、anthropic-messages，或对应的 /chat/completions、/responses、/messages 路径。'); const ops = normalized ? [{ op: 'set', path: ['providers', route, 'api'], value: normalized }] : [{ op: 'unset', path: ['providers', route, 'api'] }]; const response = await connection.api.settings.mutate({ ns: 'llm-pi-ai', ops, expectedRevision: llm.revision }); if (!response.result.ok) throw new Error(response.result.error.message); setLlm(response.result.value); notify(normalized ? '提供商默认协议已保存。' : '协议已清空，将按原始 API 地址和 Harness 默认逻辑调用。'); } catch (error) { notify(error.message, 'error'); } };
