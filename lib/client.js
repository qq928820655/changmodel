window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function toModel(model, draft) {
      ...
      return { ...model, id, name: String(draft.name || id).trim() || id, contextWindow, maxTokens, input, reasoningEfforts, ...(api ? { api } : {}) };
    }
    ...
    function Page({ connection }) {
      ...
      const saveProviderApi = async (route, api) => { try { const normalized = normalizeApi(api); if (normalized === null) throw new Error('协议必须是 openai-completions、openai-responses、anthropic-messages，或对应的 /chat/completions、/responses、/messages 路径。'); const response = await connection.api.settings.mutate({ ns: 'llm-pi-ai', ops: [{ op: 'set', path: ['providers', route, 'api'], value: normalized || 'openai-completions' }], expectedRevision: llm.revision }); if (!response.result.ok) throw new Error(response.result.error.message); setLlm(response.result.value); notify('提供商默认协议已保存。'); } catch (error) { notify(error.message, 'error'); } };
